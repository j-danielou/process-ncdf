...
HYCOM3.1_process = function(path, list_filename, varid, newvarid, domain, temp, outputDir, meridian, start, end, tempdir=NULL) {
  ...
  for (filename in list_filename){
    ...
    HYCOM = paste0("HYCOM2020", ifelse(varid_name == "temp", "sst", varid), "_temp")
    HYCOM = paste0(HYCOM, "_", sprintf("%04d", count), ".nc")
    output= file.path(output_path,HYCOM)
  ...
  if(!domain == "global" ){
    ...
    for (filename in list_filename){
      ...
      nc_subset_v2(filename=file.path(input_path,filename), varid=varid, output = file.path(output_path,filename), newvarid=newvarid, compression = 9, lat=c(-70,10), lon=c(120,300), depth=0)
    ...
    for (filename in list_filename){
      ...
      nc_subset_v2(filename=file.path(input_path,filename), varid=varid, output = file.path(output_path,filename), newvarid=newvarid, compression = 9, depth=0)
  ...
  ###   Concatenation for a Variable for HYCOM files (.nc)  ###
  
  input_path = output_path
  
  list_filename= list.files(path=input_path)
  
  output_file = paste0("gofs-3.1-",domain,"-",newvarid,"-",temp,"-",start,"-",end,".nc")
