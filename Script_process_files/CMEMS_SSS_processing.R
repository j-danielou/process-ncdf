...
CMEMS_SSS_process = function(path, list_filename, varid, newvarid, domain, temp, outputDir, meridian, start, end, tempdir=NULL) {
  ...
  for (filename in list_filename){
    ...
    GLORYS = paste0("CMEMS_SSS_", ifelse(varid_name == "thetao", "sst", varid), "_temp")
  ...
  if(!domain == "global" ){
    ...
    for (filename in list_filename){
      ...
      nc_subset_v2(filename=file.path(input_path,filename), varid=varid, output = file.path(output_path,filename), newvarid=newvarid, compression = 9, lat=c(-60,10), lon=c(120,291), depth =c(0,0.5))
  ...
  output_file = paste0("cmems-sss-",domain,"-",newvarid,"-",temp,"-",start,"-",end,".nc")
