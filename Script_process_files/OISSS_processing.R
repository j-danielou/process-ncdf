...
OISSS_process = function(path, list_filename, varid, newvarid, domain, temp, outputDir, meridian, start, end, tempdir=NULL) {
  
  source("C:/Users/jdanielou/Desktop/process-ncdf/Script_process_files/nc_extract_v2.R")
  source("C:/Users/jdanielou/Desktop/process-ncdf/Script_process_files/nc_changePrimeMeridian_oiss.R")
  ...
  for (filename in list_filename){
    ...
    OISST = paste0("OISSS", ifelse(varid_name == "temp", "sst", varid), "_temp")
  ...
  if (!meridian == "center"){
    ...
    for (filename in list_filename){
      ...
      nc_changePrimeMeridian_oiss(filename = file.path(input_path,filename), varid=varid, primeMeridian = "left", overwrite = TRUE)
    ...
    for (filename in list_filename){
      ...
      nc_changePrimeMeridian_oiss(filename = file.path(input_path,filename), varid=varid, primeMeridian = "center", overwrite = TRUE)
  ...
  if(!domain == "global" ){
    ...
    for (filename in list_filename){
      ...
      nc_subset_v2(filename=file.path(input_path,filename), varid=varid, output = file.path(output_path,filename), newvarid=newvarid, compression = 9, latitude=c(-60,10), longitude=c(120,291))
  ...
  output_file = paste0("oisss-l4-",domain,"-",newvarid,"-",temp,"-",start,"-",end,".nc")
