...
WOA2023_process = function(path, list_filename, varid, newvarid, domain, temp, outputDir, meridian, start, end, tempdir=NULL) {
  ...
  for (filename in list_filename){
    ...
    WOA = paste0("woa2023_", ifelse(varid_name == "thetao", "sst", varid), "_temp")
    WOA = paste0(WOA, "_", sprintf("%04d", count), ".nc")
    output = file.path(output_path,WOA)
  ...
  ###   Concatenation for a Variable for WOA files (.nc)  ###
