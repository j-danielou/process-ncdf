...
BRAN2020_process = function(path, list_filename, varid, newvarid, domain, temp, outputDir, meridian, start, end, tempdir=NULL) {
  ...
  for (filename in list_filename){
    ...
    BRAN = paste0("BRAN2020", ifelse(varid_name == "temp", "sst", varid), "_temp")
    BRAN = paste0(BRAN, "_", sprintf("%04d", count), ".nc")
    output= file.path(output_path,BRAN)
  ...
  if(!domain == "global" ){
    ...
    for (filename in list_filename){
      ...
      nc_subset_v2(filename=file.path(input_path,filename), varid=varid, output = file.path(output_path,filename), newvarid=newvarid, compression = 9, yt_ocean=c(-70,10), xt_ocean=c(120,300), st_ocean=c(0,3))
    ...
    for (filename in list_filename){
      ...
      nc_subset_v2(filename=file.path(input_path,filename), varid=varid, output = file.path(output_path,filename), newvarid=newvarid, compression = 9, st_ocean=c(0,3))
  ...
  ###   Concatenation for a Variable for BRAN files (.nc)  ###
  
  input_path = output_path
  
  list_filename= list.files(path=input_path)
  
  output_file = paste0("bran-2020-",domain,"-",newvarid,"-",temp,"-",start,"-",end,".nc")
