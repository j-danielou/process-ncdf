...
ARMOR_process = function(path, list_filename, varid, newvarid, domain, temp, outputDir, meridian, start, end, tempdir=NULL) {
  ...
  for (filename in list_filename){
    ...
    ARMOR = paste0("ARMOR3D_", ifelse(varid_name == "to", "sst", varid), "_temp")
    ARMOR = paste0(ARMOR, "_", sprintf("%04d", count), ".nc")
    output = file.path(output_path,ARMOR)
  ...
  ###   Concatenation for a Variable for ARMOR files (.nc)  ###
  
  input_path = output_path
  
  list_filename = list.files(path=input_path)
  
  output_file = paste0("armor-3d-",domain,"-",newvarid,"-",temp,"-",start,"-",end,".nc")
